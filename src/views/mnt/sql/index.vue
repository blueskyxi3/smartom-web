    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        <!-- 搜索 -->
         ...
         <el-select v-model="query.dbId" clearable placeholder="数据库" class="filter-item" style="width: 130px">
          <el-option v-for="item in databases" :key="item.name" :label="item.name" :value="item.id" />
        </el-select>
      ...
      <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="500px">
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
          <el-form-item label="编码" v-if="crud.status.edit" >
          ...
          <el-form-item label="SQL内容" prop="sqlText">
            <el-input :rows="3" v-model="form.sqlText" type="textarea" style="width: 370px;" />
          </el-form-item>
           <el-form-item label="规则" prop="rule">
            <el-input :rows="3" v-model="form.rule" type="textarea" style="width: 370px;" />
          </el-form-item>
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column v-if="columns.visible('id')" prop="id" label="编码" width="100" />
        <el-table-column v-if="columns.visible('name')" prop="name" label="进程名称" :show-overflow-tooltip="true"/>
        ...
        <el-table-column v-if="columns.visible('sqlText')" prop="sqlText" label="SQL内容" :show-overflow-tooltip="true" width="320"/>
        <el-table-column v-if="columns.visible('rule')" prop="rule" label="Rule" />
        <el-table-column v-if="columns.visible('alertCode')" prop="alertCode" label="告警编码">
          <template slot-scope="scope">
            {{ dict.label.alert_code[scope.row.alertCode] }}
          </template>
        </el-table-column>
        <el-table-column v-if="columns.visible('createTime')" prop="createTime" label="创建时间" width="160" :show-overflow-tooltip="true">
          <template slot-scope="scope">
        ...
        <el-table-column v-permission="['admin','sql:edit','sql:del']" label="操作" width="150px" align="center">
          <template slot-scope="scope">
            <udOperation
              :data="scope.row"
